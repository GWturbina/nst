...
// Белые БЕЗ сертификата (6 цветов: D–I, 5 чистот: IF–VS2)
const D_W = {
  IF:   [15000,13000,11000,9500,8000,6500],
  VVS1: [11000,9500,8200,7000,6000,4800],
  VVS2: [8500,7300,6400,5500,4800,3900],
  VS1:  [6500,5700,5000,4400,3800,3100],
  VS2:  [5300,4700,4100,3600,3100,2600],
}

// Белые С сертификатом (6 цветов: D–I, 5 чистот: IF–VS2)
const D_WC = {
  IF:   [18000,15500,13000,11200,9400,7600],
  VVS1: [13000,11200,9700,8300,7100,5700],
  VVS2: [10000,8600,7500,6500,5600,4600],
  VS1:  [7700,6700,5900,5200,4500,3600],
  VS2:  [6300,5500,4800,4200,3700,3100],
}
...
export function calcFancyPrice(shape, fancyColor, intensityId, clarity, carats, hasCert, regionId = 'cis', userNst = 0) {
  ...
  const shM = sm[shape] || 1.0
  const clM = { IF:1.3, VVS1:1.15, VVS2:1.08, VS1:1.0, VS2:0.92 }[clarity] || 1.0
