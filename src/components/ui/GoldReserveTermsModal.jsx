 ...
 *
 * FULLSCREEN-вариант для максимальной совместимости с мобильными WebView
 * (Telegram, SafePal). Модалка занимает весь экран через fixed inset-0.
 * Высота НЕ через vh и НЕ через проценты (они схлопываются/не резолвятся
 * в проблемных WebView) — только flex-распределение:
 *   - шапка сверху (зафиксирована)
 *   - текст в середине (скроллится, flex:1 + minHeight:0)
...
export default function GoldReserveTermsModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col" style={{ background: '#0d1a2e' }}>
      ...
      <div className={`${wrap} py-4`} style={{ ...wrapStyle, borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
        <button onClick={onClose}
