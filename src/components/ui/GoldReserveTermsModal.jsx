 ...
 *
 * Красивая центрированная карточка с затемнённым фоном, надёжная в WebView:
 *  - Portal в document.body (обходит transform-родителей, иначе fixed ломается)
 *  - высота через window.innerHeight (px), а не vh/проценты (они ломают скролл в WebView)
 *  - flex-col: шапка и кнопка зафиксированы, скроллится только текст
 */
...
export default function GoldReserveTermsModal({ isOpen, onClose }) {
  ...
  const PAD = 16 // поля вокруг карточки
  const cardMaxH = screenH ? screenH - PAD * 2 : null

  const modalContent = (
    <div
      className="fixed inset-0 z-[80] flex items-center justify-center"
      style={{ background: 'rgba(0,0,0,0.85)', height: screenH ? `${screenH}px` : '100vh', padding: PAD }}
      onClick={onClose}
    >
      <div
        ...
        style={{
          ...
          maxHeight: cardMaxH ? `${cardMaxH}px` : '90vh',
        }}
      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 flex items-center justify-between"
          ...
          <button onClick={onClose}
            className="w-9 h-9 rounded-full flex items-center justify-center text-slate-300 text-lg"
