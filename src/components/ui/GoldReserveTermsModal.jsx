 ...
 *
 * Совместима с мобильными WebView (Telegram, SafePal): скролл на оверлее,
 * без vh-единиц и вложенного flex-скролла (которые схлопывают карточку в WebView).
 *
 * Использование:
...
export default function GoldReserveTermsModal({ isOpen, onClose }) {
  ...
  return (
    // Оверлей со скроллом — занимает весь экран, скроллится сам (надёжно в WebView)
    <div
      className="fixed inset-0 z-[80] overflow-y-auto"
      style={{ background: 'rgba(0,0,0,0.9)' }}
      onClick={onClose}
    >
      {/* Центрирующая обёртка: если контент влезает — по центру, иначе сверху */}
      <div className="min-h-full flex items-start sm:items-center justify-center p-3">
        <div
          className="max-w-[480px] w-full rounded-3xl"
          style={{ background: '#0d1a2e', border: '1px solid rgba(255,215,0,0.25)' }}
