 ...
 *
 * Надёжна в мобильных WebView (Telegram, SafePal):
 *  - высота карточки через max-height:100% (НЕ vh — vh схлопывает карточку в WebView)
 *  - flex-col: шапка и кнопка зафиксированы, скроллится только текст в середине
 *  - текст всегда открывается сверху
 *
 * Использование:
...
export default function GoldReserveTermsModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[480px] rounded-3xl flex flex-col"
        style={{ background: '#0d1a2e', border: '1px solid rgba(255,215,0,0.25)', maxHeight: '100%' }}
      ...
      >
        ...
        {/* Контент — скроллится, начинается сверху */}
        <div className="px-5 py-4 overflow-y-auto" style={{ minHeight: 0, flex: '1 1 auto' }}>
