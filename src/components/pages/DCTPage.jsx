...
function BridgeSection() {
  ...
  const reload = useCallback(async () => {
    ...
    setClaimableMetals(metals || { purchaseIds: [], marketValues: [], estimatedDCT: [] })
    setBackingRate(rate)
    setLoading(false)
  }, [wallet])
  ...
  const metalIds = claimableMetals?.purchaseIds || []
  const metalValues = claimableMetals?.marketValues || []
  const metalEst = claimableMetals?.estimatedDCT || []
  const totalGemDCT = gemEst.reduce((s, v) => s + parseFloat(v || 0), 0)
  const totalMetalDCT = metalEst.reduce((s, v) => s + parseFloat(v || 0), 0)
  ...
  return (
    <div className="px-3 mt-2 space-y-2">
      ...
      <div className="p-3 rounded-2xl glass">
        <div className="flex items-center justify-between mb-2">
          <SectionTitle icon="🥇" text={`Металлы (${metalIds.length})`} color="text-yellow-400" />
          {metalIds.length > 0 && (
          ...
          <div className="space-y-1.5">
            {metalIds.map((pid, i) => (
              <div key={pid} className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                <div>
                  <span className="text-[11px] font-bold text-white">#{pid}</span>
                  <span className="text-[10px] text-slate-500 ml-2">${metalValues[i] || '0'}</span>
                </div>
                <span className="text-[11px] font-bold text-yellow-400">{parseFloat(metalEst[i] || 0).toFixed(2)} DCT</span>
              </div>
            ))}
          </div>
        )}
