 ...
 *
 * Логика цены:
 *   • Партнёр должен быть зарегистрирован в GlobalWay (проверка уровня потом)
...
export async function DELETE(request) {
  ...
  try {
    ...
    if (!wallet || !listingId) {
      return NextResponse.json({ ok: false, error: 'Нет wallet или listingId' }, { status: 400 })
    }
    ...
    if (!listing || listing.wallet !== wallet.toLowerCase()) {
