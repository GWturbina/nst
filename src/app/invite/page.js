...
function InviteContent() {
  ...
  const myLink = `${baseUrl}/invite?ref=${shareRef}`
  const shareText = '💎 Бриллианты со скидкой до 70%! Бесплатный старт + стейкинг от 50%! Присоединяйся:'
  const viberText = 'Бриллианты со скidкой до 70%! Бесплатный старт + стейкинг от 50%! Присоединяйся:'
  ...
  useEffect(() => {
    let triggered = false
    const handleMouseLeave = (e) => { if (e.clientY <= 5 && !triggered) { triggered = true; if (registered) setShowViralPopup(true); else setShowExitPopup(true) } }
    const handleBack = () => { if (registered) setShowViralPopup(true); else setShowExitPopup(true) }
  ...
  const handleJoin = () => {
    ...
    setShowExitPopup(false); setCaptureStep(1); setShowCaptureModal(true)
  }
  ...
  const submitCapture = async () => {
    ...
    if (!v.valid) { setContactError(v.error); document.getElementById('capContact').style.borderColor = '#ff4444'; return }
    setContactError(''); document.getElementById('capContact').style.borderColor = '#444'

    setSubmitting(true)
    try {
      const res = await fetch(CAPTURE_API, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ referrerId: ref, name: name || 'Гость', messenger, contact: v.normalized, pushConsent, cardId: 'dc_invite' })
      })
      const data = await res.json()
      if (data.tempId) { setTempId(data.tempId); localStorage.setItem('dc_capture_id', data.tempId) }
    } catch (e) {
      console.error('Capture error:', e)
      const localId = 'DC_TEMP_' + Date.now(); setTempId(localId); localStorage.setItem('dc_capture_id', localId)
    }
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="p-4 rounded-2xl" style={{ background: 'linear-gradient(135deg, rgba(255,215,0,0.08), rgba(245,166,35,0.08))', border: '1px solid rgba(255,215,0,0.25)' }}>
              ...
              <div className="mt-3 p-2.5 rounded-xl text-[10px] text-slate-400 leading-relaxed" style={{ background: 'rgba(255,215,0,0.05)', border: '1px solid rgba(255,215,0,0.1)' }}>
                💡 <b className="text-white">Как получить бонус:</b><br/>1. Скопируй ссылку выше<br/>2. Отправь минимум 5 друзьям<br/>3. Свяжись со своим спонсором (ID: #{ref})<br/>4. Получи персональную скидку +5-10%!
              </div>
            </div>
          </div>
        )}
          ...
          <div className="max-w-[380px] w-full p-5 rounded-3xl" style={{ background: 'linear-gradient(180deg, #1a1040, #0a0a20)', border: '1px solid rgba(168,85,247,0.3)' }}>
            {captureStep === 1 && (<>
              <div className="text-center mb-4">
                ...
                <p className="text-[12px] text-slate-400 leading-relaxed">Укажите контакт — куда прислать подробную информацию о Diamond Club и как начать зарабатывать</p>
              </div>
              ...
              <select id="capMessenger" onChange={() => { const m = document.getElementById('capMessenger')?.value||'telegram'; const inp = document.getElementById('capContact'); if(inp){inp.placeholder=PLACEHOLDERS[m]||'@username или +номер';inp.style.borderColor='#444'}; setContactError('') }}
              ...
              {contactError && <div className="text-[11px] text-red-400 mt-1 mb-2 px-1">⚠️ {contactError}</div>}
              {!contactError && <div className="h-2 mb-1"></div>}
              <label className="flex items-start gap-2.5 p-3 rounded-xl mb-4 cursor-pointer" style={{ background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.15)' }}>
                ...
                <span className="text-[11px] text-slate-400 leading-relaxed">Согласен получать уведомления о скидках и новостях Diamond Club</span>
              </label>
              <button onClick={submitCapture} disabled={submitting} className="w-full py-3.5 rounded-2xl text-[15px] font-black mb-2"
                style={{ background: submitting ? '#555' : 'linear-gradient(135deg, #a855f7, #7c3aed)', color: '#fff', border: 'none', cursor: submitting ? 'wait' : 'pointer' }}>
            ...
            {captureStep === 2 && (<>
              <div className="text-center mb-4">
                ...
                <p className="text-[12px] text-slate-400">Скоро вы получите информацию о Diamond Club</p>
              </div>
              <div className="p-4 rounded-2xl mb-4" style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.12), rgba(124,58,237,0.08))', border: '1px solid rgba(168,85,247,0.3)' }}>
                <div className="text-center mb-3">
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Запустите бот — получите инструкции, калькулятор стоимости камней и персональные уведомления</p>
                </div>
                <a href={`https://t.me/${BOT_USERNAME}?start=ref_${tempId || ref}`} target="_blank" rel="noopener noreferrer"
                  className="block w-full py-3.5 rounded-2xl text-center text-[15px] font-black mb-2" style={{ background: 'linear-gradient(135deg, #0088cc, #0066aa)', color: '#fff', textDecoration: 'none' }}>
                  🤖 Запустить бот Diamond Club
                </a>
                <div className="text-center text-[10px] text-slate-500">Нажмите Start в Telegram — бот пришлёт инструкции</div>
              </div>
              <button onClick={finishCapture} className="w-full py-3 rounded-2xl text-[14px] font-black" style={{ background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', border: 'none', cursor: 'pointer' }}>🚀 Продолжить</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="max-w-[380px] w-full p-5 rounded-3xl" style={{ background: 'linear-gradient(180deg, #1a1040, #0a0a20)', border: '1px solid rgba(255,215,0,0.2)' }}>
            <div className="text-center">
              ...
              <button onClick={handleJoin} className="w-full py-3 rounded-2xl text-base font-black mb-2" style={{ background: 'linear-gradient(135deg, #ffd700, #f5a623)', color: '#000' }}>🎁 Получить подарок + бонус!</button>
              <button onClick={() => setShowExitPopup(false)} className="text-[11px] text-slate-500" style={{ background: 'none', border: 'none', cursor: 'pointer' }}>Нет, спасибо</button>
            </div>
          </div>
        </div>
      )}
